...
const addAuthHeader = () => {
    return {
        headers: {
            'Authorization': `Bearer ${oktaAuth.client.tokenManager.get('access_token').accessToken}`
        }
    };
};
...
export const actions = {
    checkLoggedIn({ commit }) {
        if (oktaAuth.client.tokenManager.get('access_token')) {
            const idToken = oktaAuth.client.tokenManager.get('id_token');
    ...
    async getAllTodos({ commit }) {
        ...
        const response = await axios.get('/api/todo', addAuthHeader());
